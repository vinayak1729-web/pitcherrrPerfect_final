...
#    email=gemini_chat(f"""
#       Generate a personalized email for a baseball fan, {username}, related to their {question_id} answer {answer}, while also including information about the fact they follow the {team}. The email should start with a Subject: line followed by email content with HTML . The email needs to :
#       - Begin with a clear subject line about recent exciting events (in case available) or relevant baseball topic or from their  interest 
#        - Include recent and impactful player moments if related to  {answer}, even from outside  {team} , else focus on player itself, but keep relevant in baseball context.
#        - Provide engaging greetings  acknowledging user and  player {answer}, all with a passion for baseball.
#        - Create full valid `html` structure using tags such as <h1> ,<p>, `<ul>`, `<li>` for any section of content or structured output elements needed
#        - Sign off as 'By PitcherPerfect From Suryaprabha'  at bottom (in `p` tag ) . Do not use  images . Respond with final email output ONLY (with Subject: and fully formatted body). Do not return text other than Subject line at beginning, and well structured full email body with tags (with header and content all in html).
#     """)
...
# # username = "vinu"
# # question_id = "fav_player"
# # answer = "Shohei Ohtani"
# # team = "Dodgers"

# # subject, body = personalizedEmail(username, question_id, answer, team)
